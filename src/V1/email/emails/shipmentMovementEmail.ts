export const shipmentMovementEmail = (
  clientEmail: string,
  clientName: string,
  shipment: {
    id: string;
    shipmentNumber: string;
    status: string;
    way: { name: string };
  },
  clientId: string,
  domain: string,
) => ({
  toMail: clientEmail,
  subject: `Shipment Update — #${shipment.shipmentNumber}`,

  message: `
Hi ${clientName},

Your shipment has been updated.

Shipment Number: ${shipment.shipmentNumber}
Current Status: ${shipment.status}

You can view full details here:
${domain}/c/${clientId}/i/${shipment.id}

Best regards,
S3 Tracking System Team
  `,

  html: `
<div style="font-family:Arial,Helvetica,sans-serif;background:#f6f8fb;padding:40px;">
  <div style="max-width:600px;margin:auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);">

    <!-- Header -->
    <div style="background:#0f172a;padding:20px;text-align:center;">
      <h1 style="color:#ffffff;margin:0;font-size:20px;">
        📦 S3 Tracking System
      </h1>
    </div>

    <!-- Body -->
    <div style="padding:30px;">

      <h2 style="color:#111827;margin-bottom:10px;">
        Shipment Status Updated 🚚
      </h2>

      <p style="font-size:14px;color:#374151;">
        Hi <b>${clientName}</b>,
      </p>

      <p style="font-size:14px;color:#4b5563;line-height:1.7;">
        We would like to inform you that your shipment has been updated in our tracking system.
      </p>

      <!-- Shipment Box -->
      <div style="margin:20px 0;padding:15px;background:#f1f5f9;border-left:4px solid #3b82f6;border-radius:6px;">
        <p style="margin:5px 0;"><b>Shipment #:</b> ${shipment.shipmentNumber}</p>
        <p style="margin:5px 0;"><b>Status:</b> ${shipment.status}</p>
      </div>

      <!-- Status -->
      <div style="margin:20px 0;padding:15px;background:#ecfdf5;border-left:4px solid #10b981;border-radius:6px;">
        <strong style="color:#059669;">Live Update ✓</strong>
      </div>

      <!-- CTA -->
      <div style="text-align:center;margin:30px 0;">
        <a href="${domain}/c/${clientId}/i/${shipment.id}"
           style="
              background:#2563eb;
              color:#ffffff;
              padding:12px 20px;
              border-radius:8px;
              text-decoration:none;
              font-weight:bold;
              display:inline-block;
           ">
          🔍 View Shipment Details
        </a>
      </div>

      <p style="font-size:14px;color:#4b5563;line-height:1.7;text-align:center;">
        You can track real-time updates, progress, and shipment details from your dashboard page.
      </p>

      <div style="margin-top:25px;">
        <p style="font-size:13px;color:#64748b;text-align:center;">
          Thank you for choosing S3 Tracking System 🚀
        </p>
        <p style="font-weight:bold;color:#111827;text-align:center;">
          S3 Tracking System Team
        </p>
      </div>

    </div>

    <!-- Footer -->
    <div style="background:#f8fafc;padding:15px;text-align:center;font-size:12px;color:#94a3b8;">
      © ${new Date().getFullYear()} S3 Tracking System. All rights reserved.
    </div>

  </div>
</div>
  `,
});
